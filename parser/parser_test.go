...
func TestIndexExpressionParsing(t *testing.T) {
	...
	if !ok {
		t.Fatalf("indexExp is not *ast.IndexExpression, got=%T", stmt.Expression)
	}

	if !testIdentifierLiteral(t, indexExp.Left, "someArray") {
		return
	}

	if !testInfixLiteral(t, indexExp.Index, 2, "+", 4) {
		return
	}
}
