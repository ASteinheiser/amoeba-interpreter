...
func ShowAmoeba(out io.Writer) {
	...
	io.WriteString(out, "\n")
	io.WriteString(out, "              ,s##W577%@m\n")
	io.WriteString(out, "            ,#C`        ^#\n")
	io.WriteString(out, "           j#            @b\n")
	io.WriteString(out, "            #        ,,  |Q\n")
	io.WriteString(out, "            1#     ]####m \\m,    /s###MT777%N\n")
	io.WriteString(out, "             \"#    @#####p  ^7777|`         \"#\n")
	io.WriteString(out, "               %N   @####^         /Mw       @b\n")
	io.WriteString(out, "                '@p  \"WMC         dMm#b      @b\n")
	io.WriteString(out, "                 j#                     ,,,s#C\n")
	io.WriteString(out, "                 ]#               s#*7^`||^`\n")
	io.WriteString(out, "                ,#^    /#C7,     #^\n")
	io.WriteString(out, "               ##      `W##/    #^\n")
	io.WriteString(out, "               #b              #^\n")
	io.WriteString(out, "               @b           ,#M`\n")
	io.WriteString(out, "                \"@m,   ,,e#W|\n")
	io.WriteString(out, "                   \\||^|/\n")
	io.WriteString(out, "\n")
	color.ResetColor()
}
